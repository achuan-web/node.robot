...
/**
 * @param {string} path_ 
 * @param 会压缩其下包括目录的所有文件
 */
function startZIP(path_) {
    var currPath = './public/';//文件的绝对路径 当前当前js所在的绝对路径
    var targetDir = path.join(currPath, path_);
    ...
    zip.generateAsync({//设置压缩格式，开始打包
        ...
        fs.writeFileSync(currPath + "/zip/result.zip", content, "utf-8");//将打包的内容写入 当前目录下的 result.zip中
    });
}
